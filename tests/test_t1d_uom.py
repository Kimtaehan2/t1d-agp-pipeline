...
def test_closed_loop_patients_are_excluded_by_default(tmp_path):
    """2301·2307은 알고리즘이 기저를 조절한다. 수동관리 분석에 섞이면 안 된다."""
...
@requires_raw
def test_real_data_yields_twelve_manual_patients():
    """폐루프 2명(2301·2307)과 분류 불가 3명(2303·2320·2404)을 뺀 나머지."""
    result = uom.load()

    assert len(result.patients) == 12
    assert set(result.patients["modality"]) == {"MDI", "PUMP_OPEN"}
    assert "t1d_uom_2301" not in set(result.patients["patient_id"])
    assert "t1d_uom_2307" not in set(result.patients["patient_id"])


@requires_raw
def test_real_data_has_both_sensor_cadences():
    """5분(덱스콤 계열)과 15분(리브레 계열)이 섞여 있다. 하나로 가정하면 안 된다."""
    assert set(uom.load().patients["sensor_interval_min"]) == {5, 15}
