...
3. **폐루프(AID) 환자가 섞여 있다.** 2301·2307은 기저가 5분마다 1,000~2,000개의
   서로 다른 값으로 바뀐다. 사람이 손으로 넣는 값이 아니다.

**이 로더는 기본적으로 MDI 환자만 싣는다** (:data:`DEFAULT_MODALITIES`). 펌프는
기저를 기기가 쥐고 있어서 "보호자가 앱에 입력한 것처럼" 흘려 넣는 리플레이 서사와
맞지 않는다. 개방루프 펌프 4명은 ``modalities``로 명시해야 들어온다.
...
#: 환자와 22.2 mmol/L(=400.0)인 환자가 섞여 있다. 등록된 값은 넓은 쪽이고, MDI
#: 8명은 전원 27.8 쪽이라 기본 대상에서는 맞는다(27.8이 23건, 27.7은 1건).
#: 다만 22.2에서 검열된 2309(22.2가 207건)를 ``modalities``로 불러오면 그 환자의
#: 캡핑은 잡히지 않는다.
...
#: 수동관리(사람이 용량을 결정하는) 환자.
MANUAL_MODALITIES: frozenset[str] = frozenset({"MDI", "PUMP_OPEN"})

#: :func:`load`의 기본 대상. **MDI만이다.**
#:
#: 리플레이 시연은 보호자가 식사·인슐린을 앱에 입력하는 상황을 재현한다. MDI는
#: 지속형 주사 1회 + 매 끼 볼러스라 그 서사에 그대로 맞지만, 펌프는 기저를 기기가
#: 쥐고 있어 사람이 입력하지 않는다. 폐루프는 볼러스까지 알고리즘이 낸다.
DEFAULT_MODALITIES: frozenset[str] = frozenset({"MDI"})

#: MDI로 판정된 환자 8명. :data:`MODALITY`에서 파생되며, 실측 대상을 한눈에
#: 보려고 따로 둔다.
MDI_PATIENTS: tuple[str, ...] = tuple(
    sorted(pid for pid, kind in MODALITY.items() if kind == "MDI")
)
...
def load(
    ...
    Args:
        modalities: 포함할 치료 방식. 기본값 ``None``은
            :data:`DEFAULT_MODALITIES`(MDI 8명)를 뜻한다. 개방루프 펌프·폐루프·
            분류 불가 환자를 넣으려면 명시적으로 지정해야 한다.
    """
    raw_dir = Path(raw_dir) if raw_dir is not None else DEFAULT_RAW_DIR
    if not raw_dir.is_dir():
        raise FileNotFoundError(f"원본 디렉터리가 없다: {raw_dir}")

    wanted = frozenset(modalities) if modalities is not None else DEFAULT_MODALITIES
